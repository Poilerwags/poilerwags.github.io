...
function getFinalSpeed(gen, pokemon, field, side) {
    ...
    if ((pokemon.hasAbility('Unburden') && pokemon.abilityOn) ||
        ...
        (pokemon.hasAbility('Shadow Dance') && weather === 'Darkness')) {
        speedMods.push(8192);
    }
...
function getMoveEffectiveness(gen, move, type, isGhostRevealed, isGravity, isRingTarget, hasIrrelephant) {
    if ((isRingTarget || isGhostRevealed || hasIrrelephant) && type === 'Ghost' && move.hasType('Normal', 'Fighting')) {
        return 1;
    }
    else if ((isRingTarget || isGravity || hasIrrelephant) && type === 'Flying' && move.hasType('Ground')) {
        return 1;
    }
    else if ((isRingTarget || hasIrrelephant) && type === 'Ground' && move.hasType('Electric')) {
        return 1;
    }
    else if ((isRingTarget || hasIrrelephant) && type === 'Steel' && move.hasType('Poison')) {
        return 1;
    }
    else if ((isRingTarget || hasIrrelephant) && type === 'Fairy' && move.hasType('Dragon')) {
        return 1;
    }
    else if ((isRingTarget || hasIrrelephant) && type === 'Normal' && move.hasType('Ghost')) {
        return 1;
    }
    else if ((isRingTarget || hasIrrelephant) && type === 'Dark' && move.hasType('Psychic')) {
        return 1;
    }
