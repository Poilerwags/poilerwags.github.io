...
function calculateSMSSSV(gen, attacker, defender, move, field) {
    ...
    var hasIrrelephant = attacker.hasAbility('Irrelephant');
    var type1Effectiveness = (0, util_2.getMoveEffectiveness)(gen, move, defender.types[0], isGhostRevealed, field.isGravity, isRingTarget, hasIrrelephant);
    var type2Effectiveness = defender.types[1]
        ? (0, util_2.getMoveEffectiveness)(gen, move, defender.types[1], isGhostRevealed, field.isGravity, isRingTarget, hasIrrelephant)
        : 1;
    var typeEffectiveness = type1Effectiveness * type2Effectiveness;
    if (defender.teraType) {
        typeEffectiveness = (0, util_2.getMoveEffectiveness)(gen, move, defender.teraType, isGhostRevealed, field.isGravity, isRingTarget, hasIrrelephant);
    }
    ...
    else if (typeEffectiveness === 0 && move.named('Corrode')) {
    ...
    if ((defender.hasAbility('Wonder Guard') && typeEffectiveness <= 1) ||
        ...
        (move.hasType('Water') &&
            defender.hasAbility('Dry Skin', 'Storm Drain', 'Water Absorb', 'Vaporization')) ||
    ...
    if (attacker.hasAbility('Ancient Presence')) {
        typeEffectiveness = 1;
        desc.attackerAbility = attacker.ability;
    }
    if (defender.hasAbility('Omnitype')) {
        if (!attacker.hasAbility('Ancient Presence', 'Irrelephant')) {
            if (move.hasType('Rock')) {
                typeEffectiveness = 2;
            }
            else if (move.hasType('Flying', 'Fire', 'Water', 'Ice', 'Fairy', 'Crystal')) {
                typeEffectiveness = 1;
            }
            else if (move.hasType('Steel', 'Dark')) {
                typeEffectiveness = 0.5;
            }
            else if (move.hasType('Grass', 'Bug')) {
                typeEffectiveness = 1 / 16;
            }
            else if (move.hasType('Ground') && field.isGravity) {
                typeEffectiveness = 8;
            }
            else if (move.hasType('Normal', 'Fighting', 'Poison', 'Ground', 'Ghost', 'Electric', 'Psychic', 'Dragon')) {
                return result;
            }
        }
        desc.defenderAbility = defender.ability;
    ...
    var regurgDamage = 0;
    if (attacker.hasAbility('Regurgitation')) {
        var regurgMove = move.clone();
        if (attacker.named('Muk-Delta-Water')) {
            regurgMove.type = 'Water';
        }
        else if (attacker.named('Muk-Delta-Grass')) {
            regurgMove.type = 'Grass';
        }
        else if (attacker.named('Muk-Delta-Fire')) {
            regurgMove.type = 'Fire';
        }
        else if (attacker.named('Muk-Delta-Dark')) {
            regurgMove.type = 'Dark';
        }
        else if (attacker.named('Muk-Delta-Normal')) {
            regurgMove.type = 'Normal';
        }
        else if (attacker.named('Muk-Delta-Psychic')) {
            regurgMove.type = 'Psychic';
        }
        else {
            regurgMove.type = 'Ground';
        }
        var regurg1Effectiveness = (0, util_2.getMoveEffectiveness)(gen, regurgMove, defender.types[0], isGhostRevealed, field.isGravity);
        var regurg2Effectiveness = defender.types[1]
            ? (0, util_2.getMoveEffectiveness)(gen, regurgMove, defender.types[1], isGhostRevealed, field.isGravity)
            : 1;
        if (regurgMove.type === 'Ground') {
            regurgDamage = 0;
        }
        else {
            regurgDamage = Math.floor(regurg1Effectiveness * regurg2Effectiveness * defender.maxHP() / 6);
        }
        desc.attackerAbility = attacker.ability;
    }
    var damage = [];
...
function calculateBPModsSMSSSV(gen, attacker, defender, move, field, desc, basePower, hasAteAbilityTypeChange, turnOrder) {
    ...
    if ((move.named('Facade') && attacker.hasStatus('brn', 'par', 'psn', 'tox')) ||
        ...
        var type1Effectiveness = (0, util_2.getMoveEffectiveness)(gen, move, types[0], isGhostRevealed, field.isGravity, isRingTarget, hasIrrelephant);
        var type2Effectiveness = types[1] ? (0, util_2.getMoveEffectiveness)(gen, move, types[1], isGhostRevealed, field.isGravity, isRingTarget, hasIrrelephant) : 1;
    ...
    var isAttackerFairyAura = attacker.hasAbility('Fairy Aura');
    var isDefenderFairyAura = defender.hasAbility('Fairy Aura');
    var isAttackerDarkAura = attacker.hasAbility('Dark Aura');
    var isDefenderDarkAura = defender.hasAbility('Dark Aura');
    var isUserAuraBreak = attacker.hasAbility('Aura Break') || defender.hasAbility('Aura Break');
    var fairyAuraActive = (isAttackerFairyAura || isDefenderFairyAura) && move.hasType('Fairy');
    var darkAuraActive = (isAttackerDarkAura || isDefenderDarkAura) && move.hasType('Dark');
    var auraBreak = isUserAuraBreak;
    if (fairyAuraActive) {
        if (field.hasWeather('Darkness')) {
            ...
            if (auraBreak) {
                ...
                bpMods.push(5448);
                if (isAttackerFairyAura)
                    desc.attackerAbility = attacker.ability;
                if (isDefenderFairyAura)
                    desc.defenderAbility = defender.ability;
            }
        }
    }
    if (darkAuraActive) {
        if (field.hasWeather('Darkness')) {
            if (auraBreak) {
                ...
                bpMods.push(6820);
                if (isAttackerDarkAura)
                    desc.attackerAbility = attacker.ability;
                if (isDefenderDarkAura)
                    desc.defenderAbility = defender.ability;
            }
        }
        else {
    ...
    if (attacker.hasAbility('Lernean')) {
        var p = Math.floor((100 * attacker.curHP()) / attacker.maxHP());
        var damageMultiplier = p <= 20 ? 1.45 : p <= 40 ? 1.375
            : p <= 60 ? 1.3 : p <= 80 ? 1.225 : 1.15;
        bpMods.push(Math.floor(4096 * damageMultiplier));
    }
...
function calculateFinalModsSMSSSV(gen, attacker, defender, move, field, desc, isCritical, typeEffectiveness) {
    ...
    if (field.defenderSide.isReflect && move.category === 'Physical' &&
        ...
        if (field.hasWeather('Darkness')) {
            finalMods.push(field.gameType !== 'Singles' ? 2185 : 1638);
        }
        else {
