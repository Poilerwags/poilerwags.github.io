...
function getEndOfTurn(gen, attacker, defender, move, field) {
    ...
    if (field.defenderSide.isSeeded) {
        if (!defender.hasAbility('Magic Guard') && !defender.hasAbility('Omnitype')) {
            damage -= Math.floor(defender.maxHP() / (gen.num >= 2 ? 8 : 16));
    ...
    if (defender.hasType('Water') && attacker.hasAbility('Foundry')) {
        damage -= Math.floor(defender.maxHP() / 16);
    }
...
function getDescriptionLevels(attacker, defender) {
    if (attacker.level !== defender.level) {
        return [
            attacker.level === 120 ? '' : "Lvl ".concat(attacker.level),
            defender.level === 120 ? '' : "Lvl ".concat(defender.level),
        ];
    }
